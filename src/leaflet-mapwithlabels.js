...
export function initMapWithLabelsPlugin(L) {
  ...
  const MapWithLabels = L.Map.extend({
    options: {
    ...
    initialize(id, options) {
      ...
      this._labelContainer = L.DomUtil.create(
        "div",
        "",
        this.getPane(this.options.labelPane),
      );
    ...
    addLayer(layer, updateLabels = true) {
      ...
      if (!layer.getLayers && layer.options.label) {
        const layerId = layer._leaflet_id; // ID of the layer the label belongs to
        let anchor = [0, 0],
          size = [0, 0]; // icon anchor point and size (if there is an icon)
        const geomType = layer.feature ? layer.feature.geometry.type : "Marker";

        // for points with an icon or a circle, get symbol size and anchor point
        if (geomType == "Point" || geomType == "Marker") {
          anchor = layer.getIcon
            ? layer.getIcon().options.iconAnchor
            : [layer.getRadius(), layer.getRadius()];
          size = layer.getIcon
            ? layer.getIcon().options.iconSize
            : [layer.getRadius() * 2, layer.getRadius() * 2];
        }
        ...
        const latLng = layer.getLatLng
          ? layer.getLatLng()
          : geomType.endsWith("Polygon")
            ? L.PolyUtil.polygonCenter(layer._defaultShape(), this.options.crs)
            : L.LineUtil.polylineCenter(
                layer._defaultShape(),
                this.options.crs,
              );

        const pri = layer.options.labelPriority
          ? typeof layer.options.labelPriority == "function"
            ? layer.options.labelPriority(layer)
            : layer.options.labelPriority
          : 0;
        ...
        if (updateLabels) this._updateLabels();
      }
    },
    ...
    removeLayer(layer, updateLabels = true) {
      ...
      if (this._labels[layerId]) {
        ...
        if (updateLabels) this._updateLabels(); // update labels is necessary.
      }
    },

    _zoomAnim(e) {
      /** recalculating label positions during zoom animation steps */
      for (const l in this._labels) {
        const lab = this._labels[l];
        const ls = lab.span;
        if (ls) {
          const pos = this._latLngToNewLayerPoint(lab.latLng, e.zoom, e.center);
    ...
    _updateLabels() {
      ...
      const maptr = this._panes.mapPane.style.transform
        .substring(12)
        .split(", ");
      const mapx1 = -parseFloat(maptr[0]),
        mapy1 = -parseFloat(maptr[1]);
      const mapx2 = mapx1 + this._container.clientWidth,
        mapy2 = mapy1 + this._container.clientHeight;

      // placing labels in priority order
      let n = 0;
      for (let i = 0; i < this._labelPriOrder.length; i++) {
        const lab = this._labels[this._labelPriOrder[i].id]; // the label to place
        const lt =
          typeof lab.label == "function" ? lab.label(lab.layer) : lab.label; // label text
        let pos = this.latLngToLayerPoint(lab.latLng); // label reference point pixel position
        let positions = [];
        if (
          lab.layer.options.labelRepeatAlongLines &&
          lab.geomType.endsWith("LineString")
        ) {
          // labels repeated along line
          positions = this._positionsOnLinestring(
            lab.layer,
            lab.layer.options.labelRepeatDistance,
          );
        ...
        for (let j = 0; j < positions.length; j++) {
          ...
          const st =
            typeof lab.layer.options.labelStyle == "function"
              ? lab.layer.options.labelStyle(lab.layer)
              : lab.layer.options.labelStyle;
          for (const r in st) ls.style[r] = st[r];
          lab.span = ls;
          // set label text
          ls.innerHTML = lt;
          ...
          if (fits) {
            ...
            const po =
              lab.layer.options.labelPos == "auto"
                ? this._labelPosOrder
                : [lab.layer.options.labelPos];
            lab.lw = ls.clientWidth;
            lab.lh = ls.clientHeight;

            // try possible label positions
            for (const posi in po) {
              fits = true;
              const lp = po[posi];
              const p = { ...pos }; // copy position for later
          ...
          if (fits) {
            ...
            if (lab.layer.options.markerWithLabelOnly) {
              const o = lab.layer._icon || lab.layer._path;
              o.style.display = "";
              if (lab.layer._shadow) lab.layer._shadow.style.display = "";
            }
          } else {
            ...
            if (lab.layer.options.markerWithLabelOnly) {
              ...
              if (lab.layer._shadow) lab.layer._shadow.style.display = "none";
            }
          }
          lab.span.style.top = `${pos.y}px`;
          lab.span.style.left = `${pos.x}px`;
    ...
    _positionsOnLinestring(ls, dist) {
      ...
      const inbetween = (p1, p2, d) => {
        s = pdist(p1, p2);
        if (s == 0) return { x: p1.x, y: p1.y };
        return {
          x: p1.x + ((p2.x - p1.x) * d) / s,
          y: p1.y + ((p2.y - p1.y) * d) / s,
        };
      };
      // calculate bounds and return one single point if bounds are small
      const b = ls.getBounds();
      const b1 = this.latLngToLayerPoint(b.getSouthWest());
      const b2 = this.latLngToLayerPoint(b.getNorthEast());
      if (pdist(b1, b2) <= dist)
        // if the size of the line is small, simply place a label to its centre
        return [
          this.latLngToLayerPoint(
            L.LineUtil.polylineCenter(ls._defaultShape(), this.options.crs),
          ),
        ];
      // otherwise repeat it along the line
      const coords = ls._latlngs.map((latlng) =>
      ...
      function trimAt(d) {
        ...
        const pt = inbetween(coords[0], coords[1], d - sumDist);
        coords[0] = pt;
  ...
  const mapWithLabels = function (id, options) {
    return new MapWithLabels(id, options);
  };

  // SAFE CHECK: Only add to L if it's not frozen (e.g., in global script mode)
  if (Object.isExtensible(L)) {
    L.MapWithLabels = MapWithLabels;
    L.mapWithLabels = mapWithLabels;
  }

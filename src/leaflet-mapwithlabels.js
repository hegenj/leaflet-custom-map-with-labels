...
L.MapWithLabels = L.Map.extend({
    ...
    addLayer(layer, updateLabels = true) {
        ...
        if (!layer.getLayers && layer.options.label) {
            ...
            const pri = layer.options.labelPriority ? typeof layer.options.labelPriority == "function" ? layer.options.labelPriority(layer) : layer.options.labelPriority : 0;
    ...
    _intersects(bb1, bb2) {
        ...
        const b1 = L.bounds([[bb1.x1, bb1.y1], [bb1.x2, bb1.y2]]), 
    ...
    _updateLabels() {
        ...
        const maptr = this._panes.mapPane.style.transform.substring(12).split(', ');
        const mapx1 = -parseFloat(maptr[0]), mapy1=-parseFloat(maptr[1]);
        ...
        for(let i = 0; i < this._labelPriOrder.length; i++) {
            ...
            const lt = typeof lab.label == 'function' ? lab.label(lab.layer) : lab.label; // label text
            ...
            for(let j = 0; j < positions.length; j++) {
                ...
                const ls = L.DomUtil.create('span', 'leaflet-label', this._labelContainer);
                // set custom label style
                const st = (typeof lab.layer.options.labelStyle == 'function') ? lab.layer.options.labelStyle(lab.layer) : lab.layer.options.labelStyle;
                for (const r in st)
                    ls.style[r] = st[r];
                lab.span = ls;
                // set label text
                ...
                if (fits) {
                    ...
                    const po = lab.layer.options.labelPos == 'auto' ? this._labelPosOrder : [ lab.layer.options.labelPos ];
    ...
    _positionsOnLinestring(ls, dist) {
        ...
        const pdist = (p1, p2) => Math.sqrt((p2.x - p1.x)*(p2.x - p1.x) + (p2.y - p1.y)*(p2.y - p1.y));  
        ...
        const coords = ls._latlngs.map(latlng => this.latLngToLayerPoint(latlng))
